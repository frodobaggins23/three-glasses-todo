...
import { PoolScreen } from './features/pool/PoolScreen'
import { SettingsScreen } from './features/settings/SettingsScreen'
import { useAppRouter } from './hooks/useAppRouter'

function App() {
  const { screen, goHome, goAdd, goDetail, goSettings, goPool } = useAppRouter()
  ...
  return (
    ...
    >
      {screen.name === 'home' ? (
        <HomeScreen goAdd={goAdd} goDetail={goDetail} goSettings={goSettings} goPool={goPool} />
      ) : null}
      ...
      {screen.name === 'pool' ? <PoolScreen goHome={goHome} goAdd={goAdd} /> : null}
    </div>
  )
}
